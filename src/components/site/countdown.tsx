...
export function Countdown({
  ...
  if (!t) return null;
  if (t.done)
    return <span className={cn("text-sm font-semibold text-muted-foreground", className)}>Sale ended</span>;
  ...
  return (
    <div className={cn("flex items-center gap-2", className)}>
      {units.map((u) => (
        <div
          key={u.l}
          className="min-w-14 rounded-xl border border-border bg-card px-2 py-1.5 text-center shadow-soft"
        >
