...
export const Route = createFileRoute("/checkout")({
  head: () => ({
    meta: [
      { title: "Secure Checkout — Audiony Gadgets" },
      {
        name: "description",
        content:
          "Complete your Audiony Gadgets order with cash on delivery, EasyPaisa, JazzCash or bank transfer and save up to 30%.",
      },
      { property: "og:title", content: "Secure Checkout — Audiony Gadgets" },
            ...
            >
              {activeMethods.map((m) => (
                <label
                  key={m.id}
                  htmlFor={`pm-${m.id}`}
                  className={`flex cursor-pointer items-start gap-3 rounded-2xl border p-4 transition-colors ${method === m.id ? "border-primary bg-secondary" : "border-border"}`}
                >
                  ...
                  <div className="flex-1">
                    ...
                    {method === m.id ? (
                      <div className="mt-3 space-y-3 rounded-xl bg-background p-3 text-xs">
                        {m.instructions ? <p>{m.instructions}</p> : null}
                        {m.accountNumber ? (
                          <CopyRow
                            label={m.accountTitle ?? m.label}
                            value={m.accountNumber}
                          />
                        ) : null}
                        {m.iban ? <CopyRow label="IBAN" value={m.iban} /> : null}
                        {m.requiresProof ? (
                          <div>
